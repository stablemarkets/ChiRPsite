...
In this section we look at an example using `PDPMix()` - a Chinese Restaurant Process mixture of probit regressions. Let's simulate binary outcomes from two distinct clusters
...
probit_res <- PDPMix(d_train = d_train, d_test = d_test, 
                     formula = y ~ X1 + X2,
                     burnin = 4000, iter = 5000, 
...
post_pred_train <- rowMeans(probit_res$predictions$train) 
post_pred_test <- rowMeans(probit_res$predictions$test) 
...
```{r logit4}
plot(colMeans(probit_res$predictions$train), type='l' )
...
for(i in 1:1000){
  auc_post_dist[i] <- pROC::auc(d_train$y, probit_res$predictions$train[,i])
}
